// Prime Radiant gateway — built by server/scripts/build.mjs; do not edit.
...
// gateway/src/claude.ts
import { spawn } from "node:child_process";

// gateway/src/generated.ts
var SYSTEM_PROMPT = "You are the engine of Prime Radiant, an instrument for modeling decisions as probability-weighted game trees. You are a decision theorist and game theorist: rigorous, concrete, and neutral.\n\n**Your task each turn:** advance the user's model. Elicit missing structure with at most two pointed questions, or draw. Prefer drawing: emit a tree patch whenever you have enough to render something correctable. Trees must be game-theoretically coherent \u2014 decision nodes for the user, response nodes for counterparts, chance nodes for the world; probabilities that sum; payoffs at terminals in the user's chosen unit.\n\n**The user's payoffs are sovereign.** Model the utility function they state, at the numbers they state. When you must assume a number, label it `confidence:\"assumed\"` and say so in one clause. Surface *structural* problems (inconsistent probabilities, double-counted payoffs, dominated strategies, incredible threats, information the counterpart cannot have) plainly and numerically. Do not editorialize about the user's values, goals, or priorities; do not append cautions, ethical commentary, or advice they did not ask for. You are an instrument of analysis, not a counselor. If something cannot be modeled under Anthropic's usage policies, state that in one sentence and offer the closest framing you can model.\n\n**Optimality:** recommend via backward induction on the current tree. Model counterparts as rational against their stated payoffs when those are modeled, and per elicited probabilities otherwise; note which mode you used. When the user reports which branch reality took, condition on it and advise forward \u2014 no post-mortems unless asked.\n\n**Style:** plain prose, short turns, numbers inline. `say` is for the human; the `patch` is for the machine; never describe JSON in prose.\n";
var SCHEMAS = {
  "model-turn.schema.json": { "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://primeradiant.app/schema/model-turn.schema.json", "title": "ModelTurn", "description": "The structured output contract for every model turn: prose for the human, an optional patch for the machine. The client enforces this with OpenAI structured outputs (strict schema derived from this file \u2014 strict mode requires all keys present, so `patch` becomes nullable there) and retries up to 2x on validation failure with the validator error in-context. Free text never touches the tree.", "type": "object", "additionalProperties": false, "required": ["say"], "properties": { "say": { "type": "string", "description": "Plain prose for the human. Never describes JSON." }, "patch": { "type": "array", "items": { "$ref": "patch.schema.json" } } } },
  "patch.schema.json": { "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://primeradiant.app/schema/patch.schema.json", "title": "PatchOp", "description": "A single tree-mutation operation emitted by the model. Applied transactionally by the app; any invalid op rolls back the whole patch.", "oneOf": [{ "type": "object", "additionalProperties": false, "required": ["op", "tree"], "properties": { "op": { "const": "replace_tree" }, "tree": { "$ref": "node.schema.json" } } }, { "type": "object", "additionalProperties": false, "required": ["op", "parentId", "node"], "properties": { "op": { "const": "upsert_node" }, "parentId": { "type": "string" }, "node": { "$ref": "node.schema.json" } } }, { "type": "object", "additionalProperties": false, "required": ["op", "id", "fields"], "properties": { "op": { "const": "update_node" }, "id": { "type": "string" }, "fields": { "type": "object", "description": "Partial<Node> minus id/children \u2014 structural changes go through upsert/remove.", "additionalProperties": false, "minProperties": 1, "properties": { "label": { "type": "string" }, "sub": { "type": "string" }, "p": { "type": "number", "minimum": 0, "maximum": 1 }, "actor": { "enum": ["user", "counterpart", "chance"] }, "move": { "type": "string" }, "note": { "type": "string" }, "payoff": { "oneOf": [{ "type": "number" }, { "type": "object", "minProperties": 1, "additionalProperties": { "type": "number" } }] }, "confidence": { "enum": ["estimated", "user_set", "assumed"] } } } } }, { "type": "object", "additionalProperties": false, "required": ["op", "id"], "properties": { "op": { "const": "remove_node" }, "id": { "type": "string" } } }, { "type": "object", "additionalProperties": false, "required": ["op", "id"], "properties": { "op": { "const": "mark_reached" }, "id": { "type": "string" } } }, { "type": "object", "additionalProperties": false, "required": ["op", "payoffUnit"], "properties": { "op": { "const": "set_unit" }, "payoffUnit": { "$ref": "scenario.schema.json#/$defs/payoffUnit" } } }, { "type": "object", "additionalProperties": false, "required": ["op", "title"], "properties": { "op": { "const": "retitle_scenario" }, "title": { "type": "string" } } }] },
  "node.schema.json": { "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://primeradiant.app/schema/node.schema.json", "title": "Node", "description": "A node in the decision tree. `p` is the conditional probability among siblings (siblings sum ~1; the app renormalizes). Payoffs appear on terminal nodes only. Cumulative probability, EV, distributions, conditioning, and optimal-policy annotation are computed client-side and are never part of this schema.", "type": "object", "additionalProperties": false, "required": ["id", "label", "p", "actor"], "properties": { "id": { "type": "string", "description": "Stable ULID, minted client-side.", "pattern": "^[0-7][0-9A-HJKMNP-TV-Z]{25}$" }, "label": { "type": "string", "description": "\u22646-word title." }, "sub": { "type": "string", "description": "\u22648-word descriptor." }, "p": { "type": "number", "minimum": 0, "maximum": 1, "description": "Conditional probability among siblings. The root's p is 1." }, "actor": { "enum": ["user", "counterpart", "chance"], "description": "Who moves AT this node \u2014 whose options the children are (standard game-tree convention). `user` nodes take the max-EV child under backward induction; `counterpart` nodes weight by p or optimize their stated payoffs; `chance` nodes weight by p. Vestigial on terminals." }, "move": { "type": "string", "description": "The recommended action / what this branch means, one sentence." }, "note": { "type": "string", "description": "Rationale, \u22642 sentences." }, "payoff": { "description": "Terminal nodes only. Scalar in the scenario's unit, or named components (weighted scalar derived client-side).", "oneOf": [{ "type": "number" }, { "type": "object", "minProperties": 1, "additionalProperties": { "type": "number" } }] }, "confidence": { "enum": ["estimated", "user_set", "assumed"] }, "reached": { "type": "boolean" }, "children": { "type": "array", "items": { "$ref": "#" } } } },
  "scenario.schema.json": { "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://primeradiant.app/schema/scenario.schema.json", "title": "Scenario", "type": "object", "additionalProperties": false, "required": ["id", "title", "createdAt", "updatedAt", "payoffUnit", "status", "tree", "realizedPath", "transcript"], "properties": { "id": { "type": "string", "pattern": "^[0-7][0-9A-HJKMNP-TV-Z]{25}$" }, "title": { "type": "string" }, "createdAt": { "type": "string", "format": "date-time" }, "updatedAt": { "type": "string", "format": "date-time" }, "payoffUnit": { "$ref": "#/$defs/payoffUnit" }, "status": { "enum": ["modeling", "in_progress", "resolved"] }, "tree": { "$ref": "node.schema.json" }, "realizedPath": { "type": "array", "items": { "type": "string" }, "description": "Ordered node ids, root-first; empty until first mark." }, "resolvedPayoff": { "type": "number" }, "transcript": { "type": "array", "items": { "$ref": "#/$defs/message" } }, "cameraState": { "type": "object", "description": "Last framing, per device class. Opaque to the server.", "additionalProperties": true } }, "$defs": { "payoffUnit": { "type": "object", "additionalProperties": false, "required": ["kind", "label"], "properties": { "kind": { "enum": ["currency", "utils", "scale"] }, "label": { "type": "string" }, "components": { "type": "array", "minItems": 1, "items": { "type": "object", "additionalProperties": false, "required": ["name", "weight"], "properties": { "name": { "type": "string" }, "weight": { "type": "number" } } } } } }, "message": { "type": "object", "additionalProperties": false, "required": ["role", "content", "timestamp"], "properties": { "role": { "enum": ["user", "assistant"] }, "content": { "type": "string" }, "timestamp": { "type": "string", "format": "date-time" }, "patchApplied": { "type": "string", "description": "PatchId, when this turn carried a patch." } } } } }
};

// gateway/src/prompt.ts
var TURN_CONTRACT = `## turn contract
...
function assemblePrompt(context, systemPrompt) {
...
function retryFeedback(validatorError) {
  return `Your previous reply was not a valid turn: ${validatorError}. Respond again with exactly one JSON object matching the turn contract \u2014 no code fences, no prose outside it.`;
}
function retryPrompt(prompt, previousReply, validatorError) {
  return `${prompt}
...
function warmCommand(model) {
  return `claude -p --input-format stream-json --output-format stream-json --include-partial-messages --verbose --model ${model} --disallowed-tools "*"`;
}
function oneShotCommand(model, prompt) {
  return `claude -p --output-format stream-json --include-partial-messages --verbose --model ${model} --disallowed-tools "*" ` + shellQuoted(prompt);
}
function userMessageLine(prompt) {
...
function isBudgetError(text) {
  const lowered = text.toLowerCase();
  return REST_MARKERS.some((marker) => lowered.includes(marker));
}

// gateway/src/claude.ts
var BudgetRestingError = class extends Error {
  constructor() {
...
var WarmDied = class extends Error {
};
function parseStreamLine(line, stderr) {
  let object;
  try {
  ...
  switch (object.type) {
    ...
    case "rate_limit_event": {
      ...
      return /exceeded|exhausted|limit_reached|rejected/.test(flat) ? { kind: "rateLimited" } : { kind: "ignored" };
    }
    case "result": {
      ...
      if (isError || subtype !== "success") {
        ...
        return {
          kind: "error",
          error: isBudgetError(combined) ? new BudgetRestingError() : new TurnFailedError(combined)
        };
      }
...
var ClaudeRunner = class {
  ...
  spawnShell(command) {
    ...
    return spawn(shell, [...args, command], { stdio: ["pipe", "pipe", "pipe"] });
  }
  withTimeout(work, onTimeout) {
  ...
  async turn(model, prompt, onDelta) {
    try {
  ...
  async retry(model, feedback, fullRetryPrompt, onDelta) {
  ...
  warmSend(model, message, onDelta) {
...
function logLine(fields) {
  process.stdout.write(`${JSON.stringify({ ts: (/* @__PURE__ */ new Date()).toISOString(), ...fields })}
`);
}
function logRequest(route, status, startedAt) {
  logLine({ route, status, ms: Date.now() - startedAt });
}

// gateway/src/validate.ts
...
var compiled = ajv.getSchema("https://primeradiant.app/schema/model-turn.schema.json");
if (compiled === void 0) throw new Error("model-turn schema failed to compile");
var validateTurn = compiled;
function parseTurn(raw) {
  ...
  if (!validateTurn(candidate)) {
    const detail = (validateTurn.errors ?? []).map((e) => `${e.instancePath || "/"} ${e.message ?? "invalid"}`).join("; ");
    return { ok: false, error: detail || "schema validation failed" };
  }
  return {
    ok: true,
    turn: { say: candidate.say, patch: candidate.patch ?? null }
  };
}
...
function purgeCutoff(now) {
  return new Date(now.getTime() - PURGE_AFTER_DAYS * 24 * 60 * 60 * 1e3).toISOString();
}
...
var SqliteStore = class {
  ...
  get(id) {
    const row = this.db.prepare("SELECT id, title, status, updated_at, created_at, deleted_at, body FROM scenarios WHERE id = ?").get(id);
    return row === void 0 ? null : fromRow(row);
  }
  list() {
  ...
  restore(id) {
    const result = this.db.prepare("UPDATE scenarios SET deleted_at = NULL WHERE id = ? AND deleted_at IS NOT NULL").run(id);
    return Number(result.changes) > 0;
  }
  purgeSoftDeleted(now = /* @__PURE__ */ new Date()) {
    const result = this.db.prepare("DELETE FROM scenarios WHERE deleted_at IS NOT NULL AND deleted_at <= ?").run(purgeCutoff(now));
...
var JsonFileStore = class {
  ...
  list() {
    return [...this.rows.values()].filter((row) => row.deletedAt === null).sort((a, b) => a.updatedAt < b.updatedAt ? 1 : a.updatedAt > b.updatedAt ? -1 : 0);
  }
  upsert(record) {
  ...
  restore(id) {
    const row = this.rows.get(id);
    if (row === void 0 || row.deletedAt === null) return false;
...
var STATUSES = /* @__PURE__ */ new Set(["modeling", "in_progress", "resolved"]);
var MAX_BODY_BYTES = 1e6;
var SSE_KEEPALIVE_MS = 15e3;
var MAX_TURN_RETRIES = 2;
function validateScenario(s, expectedId) {
...
function validateChat(body) {
  ...
  if (b.systemPrompt !== void 0 && typeof b.systemPrompt !== "string") return null;
  if (b.focusedNodeId !== void 0 && typeof b.focusedNodeId !== "string") return null;
  return b;
}
...
function createGateway(deps) {
  ...
  async function runChatTurn(chat, send) {
    const model = chat.mode === "restructure" ? config.restructureModel : config.interactiveModel;
    const prompt = assemblePrompt(chat.context, chat.systemPrompt);
    const onDelta = (text) => send("say", { text });
    try {
      ...
      if (err instanceof BudgetRestingError) {
        ...
        send("error", { error: "turn_failed", message: err.message });
      }
    }
  }
  function handleChat(req, res, startedAt) {
...
`);
      ...
      chatQueue = chatQueue.then(() => runChatTurn(chat, send)).catch(() => send("error", { error: "turn_failed", message: "internal error" })).finally(() => {
  ...
  function handleScenarios(req, res, segments, startedAt) {
    const route = `${req.method ?? ""} /v1/scenarios`;
    const done = (status) => logRequest(route, status, startedAt);
    ...
    switch (req.method) {
      ...
      case "DELETE": {
        ...
        const deletedAt = row.deletedAt ?? (/* @__PURE__ */ new Date()).toISOString();
  ...
  return {
    handler,
    enablePairing() {
      pairingCode = String(randomBytes(4).readUInt32BE() % 1e6).padStart(6, "0");
      return pairingCode;
    },
    setBudget(state) {
      budget = state;
...
function gatewayDir() {
  return process.env.RADIANT_GATEWAY_DIR ?? path2.join(homedir(), ".prime-radiant", "gateway");
}
function loadConfig(dir) {
  const file = path2.join(dir, "config.json");
  if (!existsSync2(file)) return { ...DEFAULT_CONFIG };
...
// gateway/src/maintenance.ts
import { existsSync as existsSync3, mkdirSync as mkdirSync3, readdirSync, unlinkSync } from "node:fs";
import path3 from "node:path";
var BACKUP_KEEP_DAYS = 14;
var BACKUP_RE = /^scenarios-(\d{4}-\d{2}-\d{2})\.(sqlite|json)$/;
function dayStamp(date) {
  return date.toISOString().slice(0, 10);
}
function runMaintenance(store, backupsDir, now = /* @__PURE__ */ new Date()) {
  mkdirSync3(backupsDir, { recursive: true });
  const ext = store.kind === "sqlite" ? "sqlite" : "json";
  const dest = path3.join(backupsDir, `scenarios-${dayStamp(now)}.${ext}`);
  let backedUp = false;
  if (!existsSync3(dest)) {
    store.backup(dest);
    backedUp = true;
  }
  const cutoff = dayStamp(new Date(now.getTime() - BACKUP_KEEP_DAYS * 24 * 60 * 60 * 1e3));
  let rotated = 0;
  for (const name of readdirSync(backupsDir)) {
    const match = BACKUP_RE.exec(name);
    if (match !== null && match[1] < cutoff) {
      unlinkSync(path3.join(backupsDir, name));
      rotated += 1;
    }
  }
...
function refuseIfApiKeyPresent(env = process.env) {
  if (env.ANTHROPIC_API_KEY !== void 0 && env.ANTHROPIC_API_KEY !== "") {
    process.stderr.write(
      "FATAL: ANTHROPIC_API_KEY is set in the gateway environment. The claude CLI would silently bill API credits instead of the owner's subscription (pivot-claude-tailscale.md \xA75.2). Unset it and restart.\n"
    );
    process.exit(1);
  }
}
async function loginShellProbe(loginShell, command) {
  const [shell, ...args] = loginShell;
  try {
    const { stdout } = await execFileP(shell, [...args, command]);
    return stdout.trim();
  } catch {
    return null;
  }
...
async function main() {
  ...
  if (loginKey !== null && loginKey !== "") {
    process.stderr.write(
      "FATAL: ANTHROPIC_API_KEY is set in the box's login-shell environment. Remove it from the shell profile and restart (pivot-claude-tailscale.md \xA75.2).\n"
    );
    process.exit(1);
  }
...
`);
  ...
  const backupsDir = path4.join(dir, "backups");
  const maintain = () => {
    try {
      ...
      logLine({ event: "maintenance_failed", message: err.message });
    }
  };
  maintain();
...
var invokedDirectly = (() => {
  ...
  try {
    return import.meta.url === pathToFileURL(realpathSync(path4.resolve(process.argv[1]))).href;
  } catch {
    return false;
  }
})();
